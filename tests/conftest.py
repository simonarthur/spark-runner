"""Shared fixtures for spark_runner tests.

Sets dummy env vars BEFORE importing spark_runner so its module-level
...
# ── Set dummy env vars before the first import of spark_runner ──────────
...
import spark_runner  # noqa: E402  (must come after env-var setup)
...
def fake_tasks_dir(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Redirect ``spark_runner.TASKS_DIR`` to a temp directory."""
    d = tmp_path / "tasks"
    d.mkdir()
    monkeypatch.setattr(spark_runner, "TASKS_DIR", d)
    return d


@pytest.fixture()
def fake_goal_summaries_dir(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Redirect ``spark_runner.GOAL_SUMMARIES_DIR`` to a temp directory."""
    d = tmp_path / "goal_summaries"
    d.mkdir()
    monkeypatch.setattr(spark_runner, "GOAL_SUMMARIES_DIR", d)
    return d


@pytest.fixture()
def fake_runs_dir(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Redirect ``spark_runner.RUNS_DIR`` to a temp directory."""
    d = tmp_path / "runs"
    d.mkdir()
    monkeypatch.setattr(spark_runner, "RUNS_DIR", d)
    return d
...
def mock_summary_client(monkeypatch: pytest.MonkeyPatch) -> MagicMock:
    """Replace ``spark_runner.summary_client`` with a ``MagicMock``."""
    client = MagicMock()
    monkeypatch.setattr(spark_runner, "summary_client", client)
    return client
